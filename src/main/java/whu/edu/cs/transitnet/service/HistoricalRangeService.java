...
public class HistoricalRangeService {
    ...
    public void setup(double[] ps, String d,String st,String et){
        ...
        try {
            Date dateOrigin=dateFormat.parse(date+" 00:00:00");
    ...
    public HashSet<TripId> historaical_range_search() throws Exception {
    ...
    */
    ...
    public HashSet<TripId> spatial_hytra() throws Exception {
        int resolution = 6;
...
//                        res.addAll(generatorService.merge_CT_List.get(new CubeId(Integer.toString(zOrder))));
                    int zOrder = encodeService.combine3(i,j,k,6);
                    int level=0;
                    String cid=date+sep+zOrder+sep+level;
                    HashSet<TripId> oneCubeTrips=lsmCubeTripListSearchByCid(cid);
                    if(oneCubeTrips!=null){
                        res.addAll(oneCubeTrips);
                    }
                }
            }
        }
        hytraSerivce.getConn().close();
        return res;
    }

    private HashSet<TripId> lsmCubeTripListSearchByCid(String cid) throws Exception {
        String tripList=hytraSerivce.getConn().get(cid);
        if(tripList.contains(",")){
            String [] tripArr=tripList.split(",");
            HashSet<TripId> tripSet = new HashSet<>();

            for (String trip : tripArr) {
                tripSet.add(new TripId(trip));
            }
